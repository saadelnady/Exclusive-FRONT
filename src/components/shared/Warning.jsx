...
const Warning = ({ handleShowWarning, action }) => {
  const { Icon, message, subMessage, actionHandler } = action;
  ...
  return (
    <div className="warning-layout d-flex justify-content-center align-items-center position-fixed top-0 left-0 w-100 h-100 z-3">
      <div className="text-center p-4 bg-light col-10 col-sm-8 col-md-6 rounded">
        ...
        <h3>{message?.EN}</h3>
        {subMessage && (
          <p className="warning-sub-messge my-3">{subMessage?.EN}</p>
        )}
        <div className="d-flex align-items-center justify-content-center my-3">
          ...
          <button onClick={handleShowWarning} className="btn border">
            Cancel
          </button>
        </div>
      </div>
